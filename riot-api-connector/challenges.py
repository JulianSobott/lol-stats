    ...
    def store_challenges(self, db: db, stats: ParticipantStats, puuid: str):
        ...
        for class_ in self.classes:
            for challenge in self.classes[class_]:
                ...
                except ChallengeNotFound as exception:
                    pass  # TODO: find a better solution this is flooding the logs
                    # logger.debug(f"msg='error getting stats value' {exception=}")
        db.commit()
    ...
    def get_stat(self, challenge_name: str, stats: ParticipantStats):
        if challenge_name[0] == '-':
            ...
            try:
                ...
                raise ChallengeNotFound(challenge_name)
        else:
            if stats.challenges is None or challenge_name not in stats.challenges:
                raise ChallengeNotFound(challenge_name)
            new_stat = stats.challenges[challenge_name]
        return new_stat
...
class ChallengeNotFound(Exception):
    ...
    def __init__(self, challenge: str, *args):
        super().__init__(args)
        self.challenge = challenge

    def __str__(self):
        return f"Challenge '{self.challenge}' not found in result"
